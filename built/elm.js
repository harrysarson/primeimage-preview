...
var author$project$State$expectedChecks = function (digits) {
	var weOnlyCheckOddNumbers = 2;
	var roundSignificantFigures = F2(
		function (sigFigs, x) {
			var factor = A2(
				elm$core$Basics$pow,
				10,
				(elm$core$Basics$floor(
					A2(elm$core$Basics$logBase, 10, x)) + 1) - sigFigs);
			return elm$core$Basics$round(x / factor) * factor;
		});
	var primeProb = A2(elm$core$Basics$logBase, 10, elm$core$Basics$e) / digits;
	return A2(roundSignificantFigures, 2, (1 / primeProb) / weOnlyCheckOddNumbers);
};
		...
		switch (msg.$) {
			...
			default:
				...
				if (decoded.$ === 'Ok') {
					...
					var _n6 = A2(elm$core$Debug$log, 'status update', response);
					return A2(
						GlobalWebIndex$cmd_extra$Cmd$Extra$with,
						function () {
							switch (response.$) {
								case 'InProgress':
									var status = response.a;
									var expected = A2(
										elm$core$Maybe$withDefault,
										0,
										A2(
											...
											function (img) {
												var digits = elm$core$String$length(
													author$project$NumberString$toString(img));
												return author$project$State$expectedChecks(digits);
											},
											model.nonPrime));
									var checked = A3(
										elm$core$List$foldr,
										F2(
											function (_n8, checkedBySwapsSoFar) {
												var swapsStatus = _n8.b;
												return checkedBySwapsSoFar + A2(
													elm$core$Maybe$withDefault,
													0,
													A2(
														elm$core$Maybe$map,
														function ($) {
															return $.combinationsChecked;
														},
														swapsStatus));
											}),
										0,
										A2(elm$core$List$indexedMap, elm$core$Tuple$pair, status));
									var fraction = checked / (checked + expected);
									var squaredFraction = A2(elm$core$Basics$pow, fraction, 2);
										...
										_List_fromArray(
											[
												...
												_Utils_Tuple3(
													'.prime-progress',
													'--prime-progress',
													elm$core$String$fromFloat(squaredFraction)))
											]));
								case 'FoundPrime':
			...
			[
				...
				_List_fromArray(
					[
						...
						props.primeError),
						props.fetchingPrime ? elm$core$Maybe$Just(
						A2(
							elm$html$Html$p,
							_List_Nil,
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											elm$html$Html$Attributes$class('prime-progress')
										]),
									_List_Nil)
								]))) : elm$core$Maybe$Nothing,
						A2(
						...
						function (prob) {
							return A2(
								...
								_List_fromArray(
									[
										elm$html$Html$text('Found a prime number!'),
										elm$html$Html$text('(The probability that the number is not prime is less than 1/10^'),
										elm$html$Html$text(
										elm$core$String$fromInt(
											elm$core$Basics$round(
												((-1) * A2(elm$core$Basics$logBase, 10, elm$core$Basics$e)) * prob))),
										elm$html$Html$text('.)')
									]));
						},
...
var author$project$View$view = function (model) {
	...
	var displayProps = {
		...
		primeImage: function () {
			...
			if (_n3.$ === 'Just') {
				switch (_n3.a.$) {
					case 'FoundPrime':
						var primeNumber = _n3.a.a.primeNumber;
	...
	var interactionProps = {
		...
		primeFoundLog2Prob: function () {
			var _n2 = model.prime;
			if ((_n2.$ === 'Just') && (_n2.a.$ === 'FoundPrime')) {
				var log2ProbPrime = _n2.a.a.log2ProbPrime;
				return elm$core$Maybe$Just(log2ProbPrime);
			} else {
				return elm$core$Maybe$Nothing;
			}
		}(),
